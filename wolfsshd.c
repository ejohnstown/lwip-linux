...
static void ssh_client_connected(int lClientSocket)
{
    ...
    PwMapList pwMapList;
    word32 bufSz = 0;
    printf("\r\nCLI - Client (socket:%d) is connected", lClientSocket);
    idx = ssh_avail_session();
    if (idx < 0) {
        printf("\r\nCLI - Too many SSH connections, closing.");
    ...
    if (ret == WS_SFTP_COMPLETE) {
        if (ssh_sftp_session_active()){
            printf("\r\nOnly one SFTP session allowed at one time. Closing...");
            lwip_close(lClientSocket);
            return;
        }
        printf("\r\nSFTP client connected");
        ssh_session[idx].is_sftp = 1;
        return;
    }
    if (ret != WS_SUCCESS) {
        printf("\r\nwolfSSH_accept: error %d\n", ret);
        lwip_close(lClientSocket);
        return;
    }
    ...
    if (ssh_interactive_mode_active()) {
        ...
        printf("\r\nOnly one interactive session allowed at one time. Closing...");
    ...
    for (;;) {
        ...
        {
            lwip_close(lSocket);
            printf("\r\nCLI - Client (socket:%d) is disconnected", lSocket);
        }
        else
        {
            xByte = 0;
            while (xByte < xBytes)
            {
                ...
                if (cInChar == 0x03) { /* CTRL+C */
                    ...
                    wolfSSH_stream_send(ssh, (byte *)"\r\n", 2);
                    printf("\r\nReceived CTRL+C: Client %d is disconnected", lSocket);
                    lwip_close(lSocket);
                    break;
                }
                if (cInChar == 0x04) { /* CTRL+D */
                    ...
                    wolfSSH_stream_send(ssh, (byte *)"\r\n", 2);
                    printf("\r\nReceived CTRL+D: Client %d is disconnected", lSocket);
                    lwip_close(lSocket);
                    break;
                }
